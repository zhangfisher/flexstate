...
- 并且定义了`Initial`、`Connecting`、`Connected`、`Disconnecting`、`Disconnected`、`AlwaysDisconnected`共六个状态以及状态之间的转换约束。实质上，状态机还会自动添加一个`ERROR`和`NULL`状态。
- 定义了`connect`和`disconnect`两个动作，在这两个方法前添加`@state`代表了当执行这两个方法会导致状态变化。


# 第二步：初始化TCPSocket

当实例化`TCPClient`实例后，首先应该创建Socket实例。由于`TCPClient`实例继承自`FlexStateMachine`，并且我们指定了`Initial`为初始化状态。状态机会在实例化并启动后自动转换到`Initial`状态。因此，我们可以在进入`Initial`状态前进行初始化操作。
...
当`TCPClient`实例初始化完成后，就可以开始连接服务器。我们可以在类上创建状态机动作`connect`。
...
- `pending="Connecting"`代表，执行`connect`动作前，状态机的状态将暂时会切换至`Connecting`，也就是会显示`正在连接中`，这是比较符合实际业务逻辑的。
...
- 至此，实现了当`tcp.connect`方法，状态转换到`Connecting`状态，**连接成功**转换至`Connected`状态，**连接被断开**转换至`Disconnected`状态，**出现错误**时转换到`ERROR`状态。

# 第四步：侦听连接事件

在Tcp连接生命周期内，状态机会在最后`Initial/Connecting/Connected/Disconnecting/Disconnected/CofirmedDisconnected`状态之间进行转换，我们希望可能侦听状态机的状态转换事件，以便在连接发生状态转换时进行一些操作，此时就可以侦听各种连接事件。
...
class TcpClient extends FlexStateMachine{
  ...
  // 转换至Initial状态前会调用方法
  async onInitialEnter({retry,retryCount}){
    // 在此需要确认该切换到Disconnected还是CofirmedDisconnected状态
    this._socket.on("close",()=>{
...
class TcpClient extends FlexStateMachine{
  ...
  async login(){
    await this.send({登录数据}    
  }
  @state({
    when:["Authenticated"] 
  })
  async logout(){
    await this.send({注销数据})    
  }
}
```
