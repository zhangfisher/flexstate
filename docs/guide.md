...
# 状态机
## 转换流程
整体转换流程如下：<br />![](https://cdn.nlark.com/yuque/0/2022/jpeg/23111944/1656898288325-3a2654fc-f4b8-4572-a5fa-b376fa540de9.jpeg)

- 创建状态机时，状态机处于`NULL`状态，这是一个特殊状态，此时状态机还没有开始启动。
- 接下来调用状态机的`start`方法来启动状态机的运行。状态机运行后，首先就会转换至`initial=true`的状态。
- 接下来状态机将受`**动作**`和`**定时**`驱动进行各种业务状态之间的转换，并在状态转换期间**触发各事件和钩子**。
...
## 创建状态机
使用状态机可以有两种方式：`继承状态机类`和`直接实例化状态机`。
### 继承状态机类
...
```typescript
import { state, FlexStateMachine } from "@voerkadevice/utils/asyncStateMachine"
...
class TcpClient extends XXXX{         
   static states = {
     Initial			: {  value:0, title:"正在连接...", next:["Connected","Disconnected","Error"],initial:true },
     Connecting		: { value:1, title:"正在连接...", next:["Connected","Disconnected","Error"] },
     Connected		: { value:2, title:"已连接", next:["Disconnecting","Disconnected","Error"] },
     Disconnecting: { value:3, title:"正在断开连接...", next:["Disconnected","Error"] },
     Disconnected	: {value:4, title:"已断开连接", next:["Connecting","Error"]},
     Error				: {value:9, title:"出错", next:["*"]}        // 当错误时可以切换到任何状态
  }
  constructor(options={}){
     this._fsm = new   FlexStateMachine({ 
  ...
  // 当状态转换成功后会调用此方法
  ontTransition({from,to}){
      console.log(`从<${from}>转换到<to>`)       // 例 ==> 从<Connecting>转换到<Connected>
...
- 如果不希望自动启动状态机，则可以配置`autoStart=false`，然后就需要自行调用`start`方法来启动状态机。
```typescript
// 默认行为：自动启动状态机
...
```
## 停止状态机
当调用状态机的`stop`方法后，状态机将停止运行，此时不用再执行动作和转换状态。<br />调用`stop`方法将：
...
```
## 状态机事件
`FlexStateMachine`类继承自`eventemitter2`，本身就是一个`EventEmitter`。状态机将在工作期间触发以下事件：
...
```typescript
{
  states:{
    [状态名称]:{
      alias      : "<可选，状态别名>",                                 
      value      : <必须，状态值，Number类型>,
      title      : "<状态标题，一般用于显示>", 
      initial    : <true/false,是否是初始化状态,只能有一个状态为初始状态>, 
      final      : <true/false,最终状态>,                                                     
      enter      : ({from,to})=>{....},                        // 当进入该状态时的钩子
      leave      : ({from,to})=>{....},                        // 当离开该状态时的钩子
      done       : ({from,to})=>{....},												 // 当已切换至状态后
      resume     : ({from,to})=>{....},                        // 当离开后再次恢复时调用，详见后续说明
      next       : [] || ()=>[] || "*",                        // 定义该状态的下一个状态只能是哪些状态,也可以是返回下一个状态列表的函数,*代表可以转换到任意状态
    },
    [状态名称]:{......},
    [状态名称]:{......},  
    [状态名称]:{......},
...
## 转换约束
每个状态的`next`参数可以用来约束**该状态只能转换到其他什么状态**。<br />`next`参数取值支持：

- *****：代表可以转换至任意状态
- **[<状态名称>，....,<状态名称>]**：代表只能转换到其中任一个状态
- **函数**：代表只能转换到该函数返回的其中任一个状态
## 特殊状态
### NULL
状态机实例化后，还没有调用`start`方法前的特殊状态，代表状态机还没有开始运行。<br />当调用`start`方法后，将转换至**转换到**`initial=true`的状态。<br />并且`NULL`状态没有相对应的钩子事件和转换事件，也就是说不会触发`onNullEnter/onNullLeave/onNull`这样的钩子函数。
### INITIAL
...
# 状态转换
## 转换形式
状态转换发生一般发生在执行某个操作或者某个事件行为产生时，我们**将导致状态发生变化的操作或事件称副作用**。而副作用可能是来自外部事件，也可能是来自主动执行某些操作产生的。基于副作用的产生，状态转换存在以下两种形式：
...
## 转换过程
状态转换分三个阶段：<br />![](https://cdn.nlark.com/yuque/0/2022/jpeg/23111944/1657270270639-5aa2882f-9207-4e6f-9ab7-26156de9a139.jpeg)

- **第一步：离开当前状态 - leave**

触发`<CurrentState>/leave`事件，执行`<CurrentState>/leave`钩子函数。如果`leave`钩子函数执行成功则代表成功离开当前状态。<br />如果`leave`钩子函数执行出错，则代表无法离开当前状态，将根据错误类型来决定如何处理：

- 如果抛出的是普通的ERROR，一般代表了在钩子函数中没有产生副作用或者产生的副作用可消除，因此当前状态将保持不变。
- 如果抛出的是`SideEffectTransitionError`，则代表产生了产生不可消除的副作用，状态机将强制转换到`ERROR`状态。
- **第二步：进入目标状态前 - enter**

在转换到目标状态前，会触发`<targetState>/enter`事件，执行`<targetState>/enter`钩子函数。<br />如果`enter`钩子函数执行出错，则代表无法进入目标状态。但是由于在执行`<CurrentState>/leave`钩子函数时可能产生副作用，因此将触发`CurrentState/resume`事件，开发者应该在`CurrentState/resume`钩子函数中消除执行`<CurrentState>/leave`钩子函数时可能产生副作用。

   - 如果在`CurrentState/resume`钩子函数中成功消除了产生的副作用（没有触发错误），则将恢复到原始状态。
   - 如果 `CurrentState/resume`钩子函数中触发了错误，则说明无法消除副作用，因此状态机将转换至`ERROR`状态。
- **第三步：已转换至目标状态 - done**

当`<CurrentState>/leave`和`<targetState>/enter`均成功后，状态机将转换至目标状态，并且触发`<targetState>/done`事件。<br />注意可以订阅`<targetState>/done`事件在转换完成后做一些事情，但是无法在`done钩子`中通过触发错误来阻止转换过程。状态机将忽略done钩子函数中的所有错误。
## 转换方法
状态机提供`transition`方法来**从当前状态转换至其他状态**，该方法可以在被动场景下使用，即当接收到指定事件时调用本方法来转换状态。<br />当执行`transition`方法时，会根据`state.next`和`状态转换拦截钩子`约束下来确认是否可以成功转换到目标状态。也就是说转换能否成功，取决于目标状态是否在当前状态的`next`参数中，以及拦截钩子是否进行拦截。例如：
...
`**transition**`**方法签名如下：**
:::warning
`transition(<state>,<param={}>,options={})`
:::

- **state**：要转换的目标状态名称或状态值
- **param**：用来传递给转换钩子函数的参数，必须是一个`{}`
- **options**：可选的参数
   - **force**：不检查状态转换约束，强制转换至目标状态
```typescript
...
- **转换成功时**
   ...
   - 转换过程会触发事件
   - 依次触发钩子函数`<currentState>.leave`、 `on<currentState>Leave`、`<TargetState>.enter`、`on<TargetState>Enter`、`on<TargetState>Done`、`on<TargetState>`
- **转换失败时**
   - 触发`TransitionError` 
...
由于有限状态机同一时刻只能处于某个状态或者处于状态转换过程中。因此，不能在从A状态转换至B状态时，又同时要转换C状态。<br />通过`fsm.transitioning`属性可以查询当前是否正在转换中，如果`fsm.transitioning==true`时，再调用`fsm.transition({...})`则会触发`TransitioningError`
...
- 接下来会触发`emitAsync("A/leave")`事件，代表将离开`A`状态。所有`leave`钩子函数本质上均是订阅了`A/leave`事件，并且钩子函数可以通过返回`false`和`触发错误`来阻止离开`A`状态。`emitAsync`实质上是通过`Promise.all`调用所有钩子函数的。
- 如果所有订阅了`A/leave`事件的钩子函数均没有阻止离开A状态。接下来，应会触发`emitAsync("B/enter")`事件，所有`leave`钩子函数同样是订阅了`B/enter`事件，可以在进入B状态前做一些事件，也可以通过返回`false`和`触发错误`来阻止进入`B`状态。
...
new FlexStateMachine({ 
    states:{
        Initial:{           
            enter:async function({from,to}){...},
            leave:async ({from,to})=>{....},
            done:async ({from,to})=>{....},
            resume:async ({from,to,error})=>{....},
        },
    }
    ...
}
...
-  **定义类方法进行侦听**<br />在类中直接定义`on<State>Enter`/`on<State>Levae`/`on<State>Done`/`on<State>`/`on<State>Resume`实例方法，如:  
```typescript

class MyStateMachine extends FlexStateMachine{
    async onInitialEnter({from,to}){....}
    async onInitialLeave({from,to}){....}
    async onInitialDone({from,to}){....}
    async onInitialResume({from,to,error}){....}
    async onInitial({from,to}){....}
  
    async onConnectingEnter({from,to}){...}
    async onConnectingLeave({from,to}){...}
    async onConnectingResume({from,to,error}){....}
    async onConnectingDone({from,to})
    async onConnecting({from,to})
                                       
    async onConnectedEnter({from,to}){...}
    async onConnectedLeave({from,to}){...}
    async onConnectedResume({from,to}){...}
    async onConnected({from,to}){...}
    async onConnectedDone({from,to}){...}
    ......
}    
```
** 注意：**
...
- **在状态中直接订阅事件**
```typescript
fsm.states.Connected.on("enter",{from,to})=>{....})
fsm.states.Connected.on("leave",{from,to})=>{....})
fsm.states.Connected.on("done",{from,to})=>{....})
fsm.states.Connected.on("resume",{from,to,error})=>{....})
```

- **调用**`**on**`**方法侦听通用转换事件**<br />也可以在状态机上直接侦听相应的转换事件。  
```typescript
fsm.on("Connected/enter",{from,to})=>{....})
fsm.on("Connected/leave",{from,to})=>{....})
...
当调用`transition`方法换状态时，可以传入位置参数，例如`transition("connecting",<参数>)`，此时`onConnectingEnter({params:<参数>,from,to})`，包括三个参数:
...
当离开状态时，如`onConnectingLeave({})`，传入两个参数：
...
- **当前状态是**`**A**`**，执行**`**transition("B")**`**，先执行**`**A/leave**`**离开**`**A**`**状态，然后执行**`**B/enter**`**时出错了，也就是说无法进入到**`**B**`**状态，此时状态机应该处于什么状态？**
...
- **产生的副作用是可消除的，则就可以回退到原始状态**
- **产生的副作用是不可消除的，则转换到ERROR状态**
...
```
由于在A->B的转换过程中会先执行`A/leave`，在其中可能会产生副作用。因此在B/enter出错时（即无法进入B状态），触发`A/resume`事件，开发者可以在`A/resume`钩子中来消除副作用。<br />如果`A/resume`钩子函数中能成功消副作用，则状态机将保持在A状态；如果`A/resume`钩子函数触发了错误，则代表着无法消除副作用，因此状态机将转换到`ERROR`状态。
### 转换到ERROR状态
如果开发者明确钩子函数产生的副作用是不可消除的，则可以通过`throw new SideEffectTransitionError()`来强制转换到`ERROR`状态。此时状态机会直接触发`ERROR/done`事件，但是不会触发`<currentState>/leave`和`ERROR/enter`事件。
```typescript
class MyStateMachine extends FlexStateMachine{
    async onALeave({from,to}){
        throw new SideEffectTransitionError()
...
### 使用@state装饰器声明
```typescript
import { state, FlexStateMachine } from "asyncStateMachine"
class MyStateMachine extends FlexStateMachine{
  @state({//....动作定义....})
  async connect(param){
     // ....
...
-  `pending`、`rejected`、`rejected`、`finally`均支持指定一个函数，如下： <br />上述的`result`就是`execute`执行的返回值，因此可以动态返回一个状态值。 
```typescript
 actions:{
        connect:{  
            ...
...
- 也可以指定`retryCount`和`retryInterval`两个参数来提供动作函数的出错重试执行功能。
- `debounce`和`throttle`参数可以用来配置提供执行动作的防抖动和节流功能。
## 实例动作方法
...
```
也就是说如果上例中的`fsm.connect`方法如果执行出错，则会按`resolved/rejected/finally`配置进行状态转换，但是不会抛出错误给开发者。<br />通过配置`throwActionError`配置参数，可以允许抛出错误。
```typescript
let fsm = new MyStateMachine({
...
# 状态转换监控
除了状态转换钩子函数外，在状态转换过程还会触发转换生命周期事件：
...
当转换出错时触发，事件参数` {event:"CANCEL",error,from,to}`<br />注意，转换生命周期事件不能像钩子函数一样通过触发错误来阻止和干预转换过程，其执行错误将被忽略。<br />转换生命周期事件可以通过以下三种方式订阅。
## 在类中定义订阅
```typescript
class MyStateMachine extends FlexStateMachine{
...
```
## 记录转换历史
状态机具有一个`history`的参数，启用后可以记录状态机的转换历史记录，通过`fsm.history`可以访问。<br />`history=<数字>`代表要记录最后几条转换历史，如`history=100`代表要记录最后`100`条转换历史，超过会不记录。`fsm.history`格式如下：
```typescript
fsm.history=[
  [timestamp,state],
  [timestamp,state],
